...
presentations_count1=zeros(n/2,2); % n by 2 matrix in which first column indicate the stimulus, and the 2nd column the number of appearances.
presentations_count2=zeros(n/2,2);
presentations_count1(:,1)=(1:n/2);
presentations_count2(:,1)=(n/2+1:n);
...
while trial<=number_of_trials
    ...
    stimuli1=Shuffle(presentations_count1(presentations_count1(:,2)==min(presentations_count1(:,2)),1));
    stimuli2=Shuffle(presentations_count2(presentations_count2(:,2)==min(presentations_count2(:,2)),1));
    ...
    if sum(list2(list1==stimuli1(1))==stimuli2(1))==0
        presentations_count1(presentations_count1(:,1)==stimuli1(1),2)=presentations_count1(presentations_count1(:,1)==stimuli1(1),2)+1;
        presentations_count2(presentations_count2(:,1)==stimuli2(1),2)=presentations_count2(presentations_count2(:,1)==stimuli2(1),2)+1;
        
        %stimuli = Shuffle([stimuli1(1), stimuli2(1)]);
        list1(trial)= stimuli1(1);
        list2(trial)= stimuli2(1);
        ...
        if length(presentations_count1(presentations_count1(:,2)==min(presentations_count1(:,2)),1))==1
            trial=trial-1;
            presentations_count1(presentations_count1(:,1)==stimuli1(1),2)=presentations_count1(presentations_count1(:,1)==stimuli1(1),2)-1;
            presentations_count2(presentations_count2(:,1)==stimuli2(1),2)=presentations_count2(presentations_count2(:,1)==stimuli2(1),2)-1;
            list1(trial)=0;
        ...
        if restart==10 || time_passed>1
            presentations_count1=zeros(n/2,2); 
            presentations_count2=zeros(n/2,2);
            presentations_count1(:,1)=(1:n/2);
            presentations_count2(:,1)=(n/2+1:n);
            list1=zeros(1,number_of_trials);
