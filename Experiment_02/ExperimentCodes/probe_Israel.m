...
function probe_Israel(subjectID, order, mainPath, test_comp, sessionNum, block, numRun, numRunsPerBlock, trialsPerRun, use_eyetracker)
...
% - - - - - - - - - - - - - - -
r = Shuffle(1:4);
onsetlist = load([mainPath '/Onset_files/probe_onset_length_88_' num2str(r(1)) '.mat']);
...
if test_comp == 1
    ...
    if numRun == 1 && (block ==1||block ==3) % If this is the first run of the first or third block, show instructions
...
for trial = 1:trialsPerRun
    ...
    StimOnset = Screen(w,'Flip', runStart+onsetlist(runtrial)+baseline_fixation);
        if use_eyetracker
            %   Eyelink MSG
            % ---------------------------
            Eyelink('Message', Eventflag(GenFlags.TrialStart.str,GenFlags.Probe.str,numRun,trial,runStart));
        end
    KbQueueFlush;
    KbQueueStart;
    ...
    while noresp
        ...
        if keyIsDown && noresp
            if use_eyetracker
                    %   Eyelink MSG
                    % ---------------------------
                    Eyelink('Message', Eventflag(GenFlags.Response.str,GenFlags.Probe.str,numRun,trial,runStart));
            end
    ...
    if goodresp==1
        ...
        if use_eyetracker
            %   Eyelink MSG
            % ---------------------------
            Eyelink('Message', Eventflag(GenFlags.RespondFaster.str,GenFlags.Probe.str,numRun,trial,runStart));
        end
    ...
    if use_eyetracker
        %   Eyelink MSG
        % ---------------------------
        Eyelink('Message', Eventflag(GenFlags.Fixation.str,GenFlags.Probe.str,numRun,trial,runStart));
    end
    if goodresp ~= 1
        respTime = 999;
    end
...
if use_eyetracker
        %---------------
        %Eye-Link MSG
        Eyelink('Message', Eventflag(GenFlags.RunEnd.str,GenFlags.ProbeDemo.str,numRun,trial,runStart));
end
...
if test_comp == 1 % An fMRI experiment. Edit so that it would be good for the fmri experiment
    ...
    elseif mod(block,2) == 0 && sessionNum == 1 && numRun == numRunsPerBlock
        %if block is an even number & it's the first session & the last run
        %of the block
        CenterText(w,sprintf('Please read the next part instructions and continue on your own.') ,white,0,-270);
        CenterText(w,sprintf('Press any key to continue.') ,white,0,-170);
        Screen('Flip',w);
