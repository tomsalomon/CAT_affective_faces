...
% - - - - - - - - - - - - - - -
% binary_ranking_files = dir([path,'*binary_ranking*.txt']);
tmp = dir([outputPath '/' subjectID sprintf('_stopGoList_allstim_order%d.txt', order)]);
% tmp = dir([outputPath '/', sprintf('*%s_stopGoList_allstim_order%d.txt', subjectID, order)]);
...
freq = 22254;
    ...
    PsychPortAudio('Close', pahandle);
            % Open a new port for the next sound with low latency
            pahandle = PsychPortAudio('Open', deviceID, [], reqlatencyclass, freq, nrchannels);
            PsychPortAudio('RunMode', pahandle, 1);
            PsychPortAudio('FillBuffer', pahandle, wave);
...
if test_comp == 1
    ...
    CenterText(w,'Please focus on the image onscreen when you hear the sound.',white,0,-220);
    ...
    %     CenterText(w,'This is just a demo. Press any button to continue',Green, 0, 100);
    %     Screen('Flip',w);
