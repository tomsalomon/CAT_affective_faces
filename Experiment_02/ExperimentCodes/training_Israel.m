function [] = training_Israel(subjectID,order,mainPath,test_comp,runInd,total_num_runs_training,Ladder1IN,Ladder2IN,use_eyetracker,last_run,skip_synctest)

if nargin<11
    skip_synctest=0; 
end

% function [] = training_Israel(subjectID,order,mainPath,test_comp,runInd,total_num_runs_training,Ladder1IN,Ladder2IN)
...
Instructions_name={Instructions.name};
...
if test_comp == 1
    CenterText(w,'Please focus on the faces onscreen when you hear the sound.',white,0,-220);
...
itemnameIndex = cell(1,total_num_runs_training);
shuff_itemnameIndex = cell(1,total_num_runs_training);
...
shuff_bidValues = cell(1,total_num_runs_training);
 ...
 for runNum = runInd:last_run %this for loop allows all runs to be complete
    
    if use_eyetracker
        %   Eyelink MSG
        % ---------------------------
        Eyelink('Message', Eventflag(GenFlags.RunStart.str,GenFlags.Training.str,runNum,1,runStart));
    end
    KbQueueFlush;
    ...
    if use_eyetracker
        %   Eyelink MSG
        % ---------------------------
        Eyelink('Message', Eventflag(GenFlags.Fixation.str,GenFlags.Training.str,runNum,1,runStart));
    end
    ...
    for trialNum = 1:length(shuff_trialType{runNum})   % To cover all the items in one run.
        ...
        if use_eyetracker
            ...
            Eyelink('Message', Eventflag(GenFlags.TrialStart.str,GenFlags.Training.str,runNum,trialNum,runStart)); % mark start time in file
        end
        ...
        while (GetSecs-image_start_time < image_duration)
            ...
            if pressed && noresp
                ...
                if use_eyetracker
                    %   Eyelink MSG
                    % ---------------------------
                    Eyelink('Message', Eventflag(GenFlags.Response.str,GenFlags.Training.str,runNum,trialNum,runStart)); % mark start time in file
                end
            ...
            if  shuff_trialType{runNum}(trialNum) == 11 && (GetSecs - image_start_time >= Ladder1{runNum}(length(Ladder1{runNum}),1)/1000) && notone % shuff_trialType contains the information if a certain image is a GO/NOGO trial
                ...
                if use_eyetracker
                    %   Eyelink MSG
                    % ---------------------------
                    Eyelink('Message', Eventflag(GenFlags.CueStart.str,GenFlags.Training.str,runNum,trialNum,runStart)); % mark start time in file
                end
            ...
            end % end shuff_trialType{runNum}(trialNum) == 11 && (GetSecs - image_start_time >= Ladder1{runNum}(length(Ladder1{runNum}),1)/1000) && notone
        ...
        if use_eyetracker
            %   Eyelink MSG
            % ---------------------------
            Eyelink('Message', Eventflag(GenFlags.Fixation.str,GenFlags.Training.str,runNum,trialNum,runStart)); % mark start time in file
        end
    ...
    if use_eyetracker
        %   Eyelink MSG
        % ---------------------------
        Eyelink('Message', Eventflag(GenFlags.RunEnd.str,GenFlags.Training.str,runNum,trialNum,runStart)); % mark start time in file
    end
...
if use_eyetracker
    ...
    StopRecordTime = GetSecs-anchor;
    %   Eyelink MSG
    % ---------------------------
    % mark start time in file
    ...
    if dummymode==0
        movefile(edfFile,['./Output/', subjectID,'_Training_run_',num2str(runInd),'_to_run_',num2str(last_run),'_',timestamp,'.edf']);
    end
end
