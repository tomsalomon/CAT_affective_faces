function ScaleRanking(subjectID, use_eyetracker,skip_synctest)
%=========================================================================
...
MaxTimeRank = 2; % time until trial offset
TrialTime = 0; % time only image appear 
...
% Stimuli:
stimuli_dirs=dir( './stim/ScaleRanking/Stim_*');
StimTypeInd_list=[];
...
%---------------------------------------------------------------
fid1=fopen([OutputFolder subjectID '_Scale_Ranking_' timestamp '.txt'], 'a');
...
CenterText(w,'Ranking Images', green,0,-260);
CenterText(w,'You are about to see a set of images', white,0,-100);
CenterText(w,'For each image, please indicate how much do you like it on the', white,0,-20);
CenterText(w,'following scale, using the mouse cursor', white,0,60);
...
if use_eyetracker
    ...
    if dummymode==0
        movefile(edfFile,[OutputFolder subjectID '_Scale_Ranking_' timestamp '.edf']);
    end
end
