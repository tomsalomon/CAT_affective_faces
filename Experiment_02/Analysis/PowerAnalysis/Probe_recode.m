...
function [subject_data]=Probe_recode (subj_id_num,outpath)
...
% Deal with annoying '._' files
for datafile = length(logs):-1:1
    filename=logs(datafile).name;
    if contains(filename,'._')
        try
            delete([logs(datafile).folder,filename])
        catch
        end
            logs(datafile)=[];
    end
